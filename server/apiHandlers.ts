...
let scheduleConfigs: ScheduleConfig[] = [];

const getCronExpression = (config: ScheduleConfig) => {
...
const executeScheduledReport = async (config: ScheduleConfig) => {
...
export const postCreatePageTemplate = async (req: Request, res: Response) => {
  const { pageId, pageName } = req.body as { pageId?: string; pageName?: string };
...
export const deletePageTemplate = async (req: Request, res: Response) => {
...
export const postSchedule = async (req: Request, res: Response) => {
  const configs: ScheduleConfig[] = req.body;
...
export const triggerSchedule = async (req: Request, res: Response) => {
  ...
  let config: ScheduleConfig | undefined;
...
export const getConfig = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (result.recordset.length > 0) res.json(JSON.parse(result.recordset[0].config_value));
    else res.status(404).json({ error: 'Configuração não encontrada' });
  } catch (err: any) {
